...
export async function loadDivvyStations(map) {
  ...
  const regionURL = 'https://gbfs.divvybikes.com/gbfs/en/system_regions.json';

  try {
    const [infoRes, statusRes, freeBikeRes, regionRes] = await Promise.all([
      fetch(infoURL),
      fetch(statusURL),
      fetch(freeBikeURL),
      fetch(regionURL)
    ...
    regionLayer = L.geoJSON(
      regionData.data.regions
        .filter(r => r.region_geojson)
        .map(r => JSON.parse(r.region_geojson)),
      {
        style: {
          color: '#FF8C00',
          weight: 2,
...
export function toggleDivvyLayer(map) {
  const isVisible =
    ...
    (regionLayer && map.hasLayer(regionLayer));
  ...
  if (isVisible) {
    ...
    if (!stationLayer || !dotLayer || !ebikeLayer || !regionLayer) {
